...
// Card for live matches using the existing stream player
const LiveMatchCard: React.FC<{ match: Match }> = ({ match }) => {
  ...
  // Use match page with built-in player
  const matchUrl = `/match/${sportSlug}/${match.id}`;
...
const CDNEventsCarousel: React.FC = () => {
  ...
  // Filter to only matches with sources (streams available)
  ...
  const liveCount = useMemo(() => {
    const now = Date.now();
    return events.filter(m => m.date <= now && m.date > now - (3 * 60 * 60 * 1000)).length;
  }, [events]);
