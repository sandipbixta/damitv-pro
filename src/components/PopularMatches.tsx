...
const PopularMatches: React.FC<PopularMatchesProps> = ({
  ...
  const cleanMatches = filterCleanMatches(popularMatches.filter(match => match.sources && match.sources.length > 0) // CRITICAL: Must have sources
  ...
  React.useEffect(() => {
    const enrichMatches = async () => {
      ...
      const prioritizedMatches = liveMatches.map(match => {
      ...
      }).sort((a, b) => b.priorityScore - a.priorityScore).map(item => item.match);
